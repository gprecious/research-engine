...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (cmd === '--rebuild') {
    ...
    // Preserve anti-nag state unless a new dream has reset the cycle
    if (rebuilt.last_dream_run_id === existing.last_dream_run_id) {
      rebuilt.suggestion_shown_at = existing.suggestion_shown_at ?? null;
      rebuilt.last_shown_count = existing.last_shown_count ?? 0;
    }
    await saveLedger(rebuilt);
  } else if (cmd === '--bump') {
    // Not used in the canonical pipeline — counter is derived by --rebuild via manifest+last_dream_at.
    // Kept for manual ops / future use; the pure `bumpAfterResearch` function is unit-tested.
    process.stderr.write('ledger: --bump is not used by the canonical pipeline; --rebuild handles counter from manifest.\n');
