...
async function researchSlugsFromCwd() {
  try {
    const dirs = await fs.readdir(path.join(process.cwd(), 'research'), { withFileTypes: true });
    return dirs.filter((d) => d.isDirectory()).map((d) => `research/${d.name}`);
  } catch {
    return [];
  }
}
...
async function runCli() {
  ...
  const findings = lintVault({ pages, now: date, researchSlugs: await researchSlugsFromCwd(), logText }).findings;
