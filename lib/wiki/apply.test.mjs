...
describe('applyIngest', () => {
  ...
  it('draft=true: _drafts/concepts 에만 쓰고 index/log 는 갱신하지 않는다', async () => {
    const r = await applyIngest({ vaultDir: vault, pagePlan: plan, date: '2026-05-25', draft: true });
    expect(r.created).toContain('_drafts/concepts/mixture-of-experts.md');
    const { frontmatter } = parsePage(await read('_drafts/concepts/mixture-of-experts.md'));
    expect(frontmatter.tags).toEqual(expect.arrayContaining(['ai-generated', 'llm-wiki', 'concept']));
    await expect(read('concepts/mixture-of-experts.md')).rejects.toThrow();
    await expect(read('index.md')).rejects.toThrow();
    await expect(read('log.md')).rejects.toThrow();
  });

  it('synthesis 타입은 synthesis 디렉터리에 쓴다', async () => {
    const synthesis = structuredClone(plan);
    synthesis.pages[0] = {
      ...synthesis.pages[0],
      type: 'synthesis',
    ...
    const r = await applyIngest({ vaultDir: vault, pagePlan: synthesis, date: '2026-05-25' });
    expect(r.created).toContain('synthesis/routing-patterns.md');
    const { frontmatter } = parsePage(await read('synthesis/routing-patterns.md'));
    expect(frontmatter.tags).toEqual(expect.arrayContaining(['ai-generated', 'llm-wiki', 'synthesis']));
  });

  it('CLI --draft: _drafts 경로에 쓴다', async () => {
    const planPath = path.join(vault, 'plan.json');
    await fs.writeFile(planPath, JSON.stringify(plan));
    const { stdout } = await execFileAsync(process.execPath, [
      path.join(process.cwd(), 'lib/wiki/apply.mjs'),
      '--vault', vault,
      '--plan', planPath,
      '--date', '2026-05-25',
      '--draft',
    ]);
    expect(JSON.parse(stdout).created).toContain('_drafts/concepts/mixture-of-experts.md');
    await expect(read('_drafts/concepts/mixture-of-experts.md')).resolves.toMatch(/ai-generated/);
  });
});
