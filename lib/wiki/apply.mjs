...
export async function applyIngest({ vaultDir, pagePlan, date, draft = false }) {
  // 1) 검증·준비 전체 (디스크 쓰기 전)
  const prepared = [];
  for (const p of pagePlan.pages) {
    ...
    const rel = `${draft ? '_drafts/' : ''}${DIR[p.type]}/${p.slug}.md`;
  ...
  if (draft) return { source: pagePlan.source, created, merged };
