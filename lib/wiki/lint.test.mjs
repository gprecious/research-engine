...
describe('lintVault librarian rules', () => {
  ...
  it('discoverResearchSlugs: _index 같은 비세션 디렉터리는 제외', async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'wiki-lint-'));
    await fs.mkdir(path.join(tmp, 'research/2026-06-01-session'), { recursive: true });
    await fs.mkdir(path.join(tmp, 'research/_index'), { recursive: true });
    await fs.writeFile(path.join(tmp, 'research/2026-06-01-session/README.md'), '# session\n');
    await fs.writeFile(path.join(tmp, 'research/_index/manifest.json'), '{}\n');

    await expect(discoverResearchSlugs(tmp)).resolves.toEqual(['research/2026-06-01-session']);
  });
});
