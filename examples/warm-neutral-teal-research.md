...
<link href="https://fonts.googleapis.com/css2?family=Fraunces:wght@700&family=Inter:wght@400;700&display=swap" rel="stylesheet">
...
section.title h1 { font-size: 88pt; line-height: 0.96; }
section h1 { font-size: 44pt; }
section h2 { font-size: 32pt; }
section strong { color: var(--a1); font-weight: 700; }
...
section.divider { background: var(--bg); color: var(--text); display: flex; flex-direction: column; justify-content: center; border-left: 24px solid var(--a2); padding-left: 120px; }
section.divider h1 { font-size: 96pt; color: var(--text); }
section.divider p { font-family: var(--body-font); font-size: 22pt; color: var(--a2); font-weight: 700; letter-spacing: 0.12em; text-transform: uppercase; margin-top: 24px; }
section.bento { display: grid; grid-template-columns: 1.15fr 1fr; gap: 56px; align-content: center; }
section.bento h1 { font-size: 40pt; }
section.chart-hero { padding: 56px 72px; }
...
section::after { color: var(--a2); opacity: 0.55; font-family: var(--heading-font); font-size: 12pt; }
</style>
...
단일 Marp 템플릿 + QuickChart GET만 돌리면 모든 주제가 동일한 레이아웃을 받는다.
...
## **5개 참고 스킬**이 훔칠 레시피를 정의한다

각 레포는 하나씩 다른 축을 해결한다 — 예제·QA·디스커버리·백엔드·평가.

- robonuggets: 22 예제 [23]
- ryanbbrown: Playwright QA [24]
- zarazhangrui: 3-preview [25]
- zl190: 4-backend dispatcher [26]
...
- P0 — 제목 규약 + Design Rules 8개 [23,27]
